...
decl_storage! {
	trait Store for Module<T: Config> as SimpleMap {
		...
		VotingAdm get(fn adm_map): map hasher(blake2_128_concat)  bool => T::AccountId;
	}
}
...
decl_event!(
		...
		AccountId = <T as frame_system::Config>::AccountId,
	{
		...
		EntryAdm(bool, AccountId),

		EntryAdmGot(AccountId, u32),
	}
);
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		#[weight = 10_000]
		fn set_adm_voting(origin, account: T::AccountId) -> DispatchResult {
			// A user can only set their own entry
			let user = ensure_signed(origin)?;

			<VotingAdm<T>>::insert(true, account);

			Self::deposit_event(RawEvent::EntryAdm(true,user));
			Ok(())
		}
